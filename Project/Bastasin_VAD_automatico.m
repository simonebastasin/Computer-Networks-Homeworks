...
% 5: numero dei file audio
for n = 1:5
    
    nameInputFile = strcat("inputaudio", num2str(n), ".data");
    nameOutputFile = strcat("outputVAD", num2str(n), ".txt");
    ...
    % ultimo pacchetto: non contenente 160 byte
    output(i+L:endVect) = zeros(1, endVect-(i+L)+1, 'int8');
    fwrite(outputFile, '0', "int8");
    % chiusura il file output
    fclose(outputFile);
    
    % file con pacchetti in output per convertire il risultato in .wav
    % nameEndFile = strcat("outputaudio", num2str(n), "_", num2str(soglia), ".data");
